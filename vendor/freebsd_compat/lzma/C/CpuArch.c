...
// PATCHED: Only use HWCAP on Linux (asm/hwcap.h is Linux-specific)
// FreeBSD/NetBSD would need elf_aux_info() instead of getauxval()
#ifdef __linux__
#include <sys/auxv.h>
...
#else // !__linux__ (BSD platforms)
// PATCHED: Stub implementations for BSD (would need elf_aux_info() for runtime detection)
